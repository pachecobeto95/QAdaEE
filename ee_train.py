...
def main(args):
	...
	dataset_path = os.path.join(config.DIR_PATH, "datasets")

	train_loader, test_loader = utils.load_cifar10(args.batch_size_train, dataset_path)
	...
	while (count_patience < 10):
		epoch += 1

		train_result = trainEEDNNs(ee_model, train_loader, optimizer, criterion, n_exits, epoch, device, loss_weights)
		val_result = evalEEDNNs(ee_model, test_loader, criterion, n_exits, epoch, device, loss_weights)
		#lr_scheduler.step()
