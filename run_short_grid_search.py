...
def extract_SMDP_params(df, B, lambda_rate, K=100):
  Params = namedtuple("Params", ["B", "mu_a", "mu_b", "lambda_rate", "p_a", "p_b"])

  #print(df.mu_a, df.mu_b, df.packet_loss_a, df.packet_loss_b)
  params = Params(B=B, mu_a=1000*df.mu_a.item(), mu_b=1000*df.mu_b.item(),
                  lambda_rate=lambda_rate, p_a=df.packet_loss_a.item(), p_b=df.packet_loss_b.item())
  return params


def save_QAdaEE_results(threshold_mc_overhead_list, tau_mc_overhead_list, class_name_list, overhead_list, 
    lambda_rate, factor, params_overhead_list, B, results_path):

    results_dict = {}

    for i, overhead in enumerate(overhead_list):
        for j, class_name in enumerate(class_name_list):
            results_dict.update({"mc_threshold":[threshold_mc_overhead_list[i][j]], 
                "tau": [tau_mc_overhead_list[i][j]], "overhead": [overhead], "lambda_rate": [lambda_rate],
                "class_name": [class_name], "factor": [factor], "B": [B],
                "mu_a": [params_overhead_list[i][j].mu_a], "mu_b": [params_overhead_list[i][j].mu_b], 
                "p_a": [params_overhead_list[i][j].p_a], "p_b": [params_overhead_list[i][j].p_b]})
...
save_params_path = os.path.join(config.DIR_PATH, "mobilenet", "results",
	"qAdaEE_params_mobilenet_1_branches_crescent_id_3_cifar10_vfinal3.csv")

df_params = pd.read_csv(save_params_path)

class_name_list = df_params.class_name.unique()

buffer_size_list = [100, 1000]

lambda_rate_list = [1, 10]

gamma, max_good_configs = 0.05, 5

overhead_list, factor_list = [0, 10, 100], np.round(np.arange(0.01, 0.2, 0.01), 2)

for B in tqdm.tqdm(buffer_size_list):
    results_path = os.path.join(config.DIR_PATH, "mobilenet", "inference_data", 
        "qAdaEE_short_results_mobilenet_1_branches_3_id_crescent_cifar10_buffer_%s_full_vfinal3.csv"%(B))

    good_configs_count = 0

    for lambda_rate in tqdm.tqdm(lambda_rate_list):

        for factor in tqdm.tqdm(factor_list):

            threshold_mc_overhead_list, tau_mc_overhead_list, params_overhead_list = [], [], []


            for overhead in overhead_list:
                threshold_mc_classes_list, tau_mc_list, params_list = [], [], []
                ...
                for class_name in class_name_list:
                    ...
                    threshold_mc_classes_list.append(threshold_mc), tau_mc_list.append(tau_mc), params_list.append(params)

                    #vi_results = multi_operator_value_iteration(params, gamma)

                threshold_mc_overhead_list.append(threshold_mc_classes_list), tau_mc_overhead_list.append(tau_mc_list)
                params_overhead_list.append(params_list)
            ...
            if(good_policy):
                print("GOOD POLICY")
                print(params)
                good_configs_count += 1
                save_QAdaEE_results(threshold_mc_overhead_list, tau_mc_overhead_list, class_name_list, 
                    overhead_list, lambda_rate, factor, params_overhead_list, B, results_path)

            if (good_configs_count == max_good_configs):
                break
